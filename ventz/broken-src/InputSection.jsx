...
function InitialScreen({ getCombinedMessage, handleSend, hasFiles, loadingFiles }) {
  const handleExamplePrompt = (prompt) => {
    if (hasFiles()) {
      if (loadingFiles) {
        alert("Please wait while the files are still being processed.");
      } else {
        // Combine the input box content, parsed file content, and button prompt
        const combinedMessage = getCombinedMessage(prompt);
        console.log("Combined message set for sending: ", combinedMessage);  // Debugging log for combined message

        // Ensure the fully combined message (input + parsed content + prompt) is sent
        handleSend(combinedMessage);  // Send the combined message
      }
    } else {
